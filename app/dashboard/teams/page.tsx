...
export default function TeamsPage() {
    ...
    const fetchAvailableEmployees = async (teamId: number, search: string = '') => {
        try {
    ...
    const getTeamTypeBadgeColor = (type: string | null) => {
        ...
        return type ? (colors[type] || 'bg-gray-900 text-gray-300') : 'bg-gray-900 text-gray-300';
    };
        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-600">Create and manage teams with members</p>
                </div>
                <button
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-gray-200">
                                        ...
                                        <th className="text-left py-3 px-4 text-sm font-semibold text-gray-700">Team Leader</th>
                                ...
                                <tbody>
                                    {teams.map((team) => (
                                        <tr key={team.teamno_pk} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                            <td className="py-4 px-4 text-gray-600">{team.teamcode || '-'}</td>
                                            <td className="py-4 px-4">
                                                <div className="font-medium text-gray-900">{team.teamname}</div>
                                            </td>
                                            <td className="py-4 px-4">
                                                {team.teamtype ? (
                                                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getTeamTypeBadgeColor(team.teamtype)}`}>
                                                        {team.teamtype}
                                                    </span>
                                                ) : (
                                            ...
                                            <td className="py-4 px-4">
                                                {team.teamlead_name ? (
                                                    <div>
                                                        <div className="font-medium text-gray-900">{team.teamlead_name}</div>
                                                        <div className="text-xs text-gray-500">{team.teamlead_email}</div>
                                                    </div>
                                                ) : (
                        ...
                        {pagination.totalPages > 1 && (
                            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200">
                                <div className="text-sm text-gray-700">
                                    ...
                                    <span className="font-medium">{Math.min(currentPage * itemsPerPage, pagination.totalTeams)}</span> of{' '}
                                    <span className="font-medium">{pagination.totalTeams}</span> teams
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setFormData({ ...formData, teamlead_empno_fk: e.target.value ? parseInt(e.target.value) : null })}
                                        className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-[#468847]"
                                    >
                                ...
                                <div>
                                    ...
                                    <div className="space-y-2 max-h-96 overflow-y-auto">
                                        {teamMembers.length === 0 ? (
                                            <p className="text-gray-500 text-sm">No members yet. Add members from the right panel.</p>
                                        ) : (
                                            teamMembers.map((member) => (
                                                <div key={member.teamdtlno_pk} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
                                                    <div className="flex-1">
                                                        <div className="font-medium text-gray-900">{member.employee_name}</div>
                                                    ...
                                                    <button
                                                        onClick={() => handleRemoveMember(member.teamdtlno_pk)}
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            fetchAvailableEmployees(selectedTeam.teamno_pk, e.target.value);
                                        }}
                                    ...
                                    <div className="space-y-2 max-h-96 overflow-y-auto">
                                        {availableEmployees.length === 0 ? (
                                            ...
                                            availableEmployees.map((employee) => (
                                                <div key={employee.empno_pk} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
                                                    <div className="flex-1">
                                                        <div className="font-medium text-gray-900">{employee.name}</div>
                                                        <div className="text-xs text-gray-500">{employee.emailidoffical}</div>
                                                        <div className="text-xs text-gray-400">
